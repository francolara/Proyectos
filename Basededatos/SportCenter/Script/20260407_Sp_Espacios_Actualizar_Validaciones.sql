...
-- Firma: Codex - 07/04/2026 | Bloquea cambio a mantenimiento/inactivo cuando existen reservas activas futuras en la sede.
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            WHERE e2.SedeId = @SedeActualId
              AND r.Estado IN (1, 2, 3, 4)
              AND (r.Fecha > @Hoy OR (r.Fecha = @Hoy AND r.HoraFin > @HoraActual));

            IF @ReservasActivas IS NOT NULL
                RAISERROR('No se puede cambiar a mantenimiento/inactivo. La sede tiene reservas activas futuras: %s. Cancela esas reservas para realizar la accion.', 16, 1, @ReservasActivas);
        END;
