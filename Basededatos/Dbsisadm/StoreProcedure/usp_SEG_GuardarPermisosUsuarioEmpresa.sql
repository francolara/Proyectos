...
-- Description:   Guarda en una sola transaccion los overrides de permisos de una empresa asignada al usuario.
-- =============================================

CREATE OR ALTER PROCEDURE dbo.usp_SEG_GuardarPermisosUsuarioEmpresa
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            RAISERROR (N'La empresa no esta asignada al usuario o la asignacion esta inactiva.', 16, 1);
            RETURN;
        END;
        ...
        BEGIN
            RAISERROR (N'Uno o mas modulos no pertenecen al alcance EMPRESA o estan inactivos.', 16, 1);
            RETURN;
        END;

        BEGIN TRANSACTION;

        MERGE dbo.SEG_UsuarioEmpresaPermiso AS destino
        USING
        (
            SELECT
                p.IdModuloSistema,
            ...
            ON destino.IdUsuarioEmpresa = @IdUsuarioEmpresa
        ...
        FROM dbo.SEG_UsuarioEmpresaPermiso AS permisoUsuario
        INNER JOIN @Permisos AS permisoEntrada
            ON permisoEntrada.IdModuloSistema = permisoUsuario.IdModuloSistema
        WHERE permisoUsuario.IdUsuarioEmpresa = @IdUsuarioEmpresa
