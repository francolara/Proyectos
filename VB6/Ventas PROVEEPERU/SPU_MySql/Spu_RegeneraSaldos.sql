DROP PROCEDURE IF EXISTS `Spu_RegeneraSaldos`;
DELIMITER $$

CREATE PROCEDURE `Spu_RegeneraSaldos`(
VarIdEmpresa                 Char(2),
    VarFecha                 VarChar(50)
    )
BEGIN    DECLARE PeriodoAnt        VARCHAR(50);
SET PeriodoAnt =   (SELECT DATE_FORMAT(DATE_add(VarFecha, INTERVAL -1 month), '%Y%m'));
-- ELIMINAMOS EL LA DATA DEL PERIODO ACTUAL
DELETE FROM tbsaldo_costo_kardex    WHERE Sc_Periodo = DATE_FORMAT(VarFecha, '%Y%m')
AND (idEmpresa = VarIdEmpresa OR VarIdEmpresa  = '');
 ...
 SELECT DATE_FORMAT(VarFecha, '%Y%m'),sc_Codalm,Sc_Codart,Sc_Stock,idempresa
 ...
 WHERE (A.IdEmpresa = VarIdEmpresa OR VarIdEmpresa  = '')    AND A.EstValeCab <> 'ANU'
 AND  DATE_FORMAT(A.fechaemision, '%Y%m')  = DATE_FORMAT(VarFecha, '%Y%m')    AND  A.fechaemision  < VarFecha
  ...
  and A.Sc_Codart = B.IdProducto and A.idempresa = B.idempresa    SET A.Sc_Stock = A.Sc_Stock + B.Stock
   WHERE A.Sc_periodo = DATE_FORMAT(VarFecha, '%Y%m') ;
    ...
    SELECT DATE_FORMAT(VarFecha, '%Y%m'),a.idalmacen,a.IdProducto,a.Stock,a.idempresa
     ...
     LEFT JOIN tbsaldo_costo_kardex B
     ON  b.Sc_periodo =  a.Periodo and b.sc_Codalm =  a.idalmacen
     AND b.Sc_Codart = a.IdProducto
     and b.idempresa = a.idempresa
