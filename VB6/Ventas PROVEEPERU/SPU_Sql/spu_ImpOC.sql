...
-- exec spu_ImpOC '01','08090001','94','999 ','00000002'
...
BEGIN
    ...
    SELECT  c.idDocVentas,c.GlsCliente, c.RUCCliente,c.dirCliente,CONVERT(VARCHAR,C.FecEmision,103) AS FecEmision,c.TotalValorVenta,
			...
			D.Cantidad,D.GlsProducto,D.idProducto,d.GlsUM,D.item,D.VVUnit AS VVUnit,D.PorDcto,D.TotalVVNeto AS TotalVVNeto,
           CASE c.idMoneda 
               WHEN 'USD' THEN 'US$' 
               ELSE 'S/.' 
           END AS Moneda,
		   ...
		   CONVERT(VARCHAR,C.FecIniTraslado,103) AS FecIniTraslado,
		   Partida2
