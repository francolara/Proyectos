...
Begin VB.Form FrmComunicacionBaja 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin DXDBGRIDLibCtl.dxDBGrid G 
         Height          =   3885
         Left            =   60
         OleObjectBlob   =   "FrmComunicacionBaja.frx":0000
         TabIndex        =   10
         Top             =   150
         Width           =   13785
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton CmbAyudaTipoDoc 
         ...
         Height          =   315
         Left            =   7095
         Picture         =   "FrmComunicacionBaja.frx":3D36
         Style           =   1  'Graphical
         TabIndex        =   1
         Top             =   210
         Width           =   390
      ...
      Begin CATControls.CATTextBox TxtIdDocumento 
         ...
         MaxLength       =   8
         Container       =   "FrmComunicacionBaja.frx":40C0
         Estilo          =   1
         EnterTab        =   -1  'True
      End
      Begin CATControls.CATTextBox TxtGlsDocumento 
         ...
         FontSize        =   8.25
         ForeColor       =   -2147483640
         Container       =   "FrmComunicacionBaja.frx":40DC
         Vacio           =   -1  'True
      End
   ...
   End
End
Attribute VB_Name = "FrmComunicacionBaja"
...
Dim RetVal
    ...
    strRuta = CCarpeta & "\" & traerCampo("Empresas", "Ruc", "IdEmpresa", glsEmpresa, False) & "-RA-" & Format(getFechaSistema, "yyyymmdd") & "-2.xml"
    IntFile = FreeFile
    Open strRuta For Output As #IntFile
        
    strLinea = ""
    strLinea = strLinea & "<?xml version=""1.0"" encoding=""ISO-8859-1"" standalone=""no""?> " & vbCrLf
    strLinea = strLinea & "<VoidedDocuments " & vbCrLf
    strLinea = strLinea & "    xmlns = ""urn:sunat:names:specification:ubl:peru:schema:xsd:VoidedDocuments-1"" " & vbCrLf
    ...
    strLinea = strLinea & "    <cbc:ID>RA-" & Format(getFechaSistema, "yyyymmdd") & "-2</cbc:ID> " & vbCrLf
    ...
    If G.Columns.ColumnByFieldName("Ck").Value = "1" Then
    NItem = NItem + 1
    strLinea = strLinea & "    <sac:VoidedDocumentsLine> " & vbCrLf
    strLinea = strLinea & "        <cbc:LineID>" & NItem & "</cbc:LineID>" & vbCrLf
    strLinea = strLinea & "        <cbc:DocumentTypeCode>" & TxtIdDocumento.Text & "</cbc:DocumentTypeCode> " & vbCrLf
    strLinea = strLinea & "        <sac:DocumentSerialID>" & G.Columns.ColumnByFieldName("IdSerie").Value & "</sac:DocumentSerialID> " & vbCrLf
    strLinea = strLinea & "        <sac:DocumentNumberID>" & G.Columns.ColumnByFieldName("IdDocVentas").Value & "</sac:DocumentNumberID> " & vbCrLf
    strLinea = strLinea & "        <sac:VoidReasonDescription>" & G.Columns.ColumnByFieldName("GlsObsAnula").Value & "</sac:VoidReasonDescription> " & vbCrLf
    strLinea = strLinea & "    </sac:VoidedDocumentsLine> " & vbCrLf
    ...
    strLinea = strLinea & "</VoidedDocuments> "
    ...
    RetVal = ShellExecute(Me.hwnd, "Open", App.Path & "\Release\FEapp2.exe", "", "", 0)
...
Dim StrMsgError                         As String
    ...
    ConfGrid G, True, True, False, False
    DtpFecha.Value = getFechaSistema
    TxtIdDocumento.Text = "01"
...
Dim rsg                         As New ADODB.Recordset
    ...
    rsg.Fields.Append "CK", adInteger, 14, adFldIsNullable
    rsg.Fields.Append "Item", adInteger, 14, adFldIsNullable
    rsg.Fields.Append "IdSerie", adVarChar, 4, adFldIsNullable
    rsg.Fields.Append "IdDocVentas", adVarChar, 8, adFldIsNullable
    rsg.Fields.Append "FecEmision", adVarChar, 10, adFldIsNullable
    rsg.Fields.Append "RucCliente", adVarChar, 11, adFldIsNullable
    rsg.Fields.Append "GlsCliente", adVarChar, 250, adFldIsNullable
    rsg.Fields.Append "IdMoneda", adVarChar, 3, adFldIsNullable
    rsg.Fields.Append "GlsObsAnula", adVarChar, 200, adFldIsNullable
    rsg.Fields.Append "TotalPrecioVenta", adDouble, 14, adFldIsNullable
    rsg.Open
        
    CSqlC = "Select 0 Ck,@i:=@i+1 Item,A.IdSerie,A.IdDocVentas,A.FecEmision,A.RucCliente,A.GlsCliente,A.IdMoneda,A.GlsObsAnula,A.TotalPrecioVenta " & _
            "From (Select @i:=0) Z,DocVentas A " & _
            "Where A.IdEmpresa = '" & glsEmpresa & "' And A.IdDocumento = '" & TxtIdDocumento.Text & "' And A.EstDocVentas = 'ANU' " & _
            "And A.IndBajaSunat = 0 And A.FecEmision = '" & Format(DtpFecha.Value, "yyyy-mm-dd") & "'"
    ...
    If Not RsC.EOF Then
        ...
        Do While Not RsC.EOF
            ...
            rsg.AddNew
            rsg.Fields("CK") = RsC.Fields("CK")
            rsg.Fields("Item") = RsC.Fields("Item")
            rsg.Fields("IdSerie") = RsC.Fields("IdSerie")
            rsg.Fields("IdDocVentas") = RsC.Fields("IdDocVentas")
            rsg.Fields("FecEmision") = RsC.Fields("FecEmision")
            rsg.Fields("RucCliente") = RsC.Fields("RucCliente")
            rsg.Fields("GlsCliente") = RsC.Fields("GlsCliente")
            rsg.Fields("IdMoneda") = RsC.Fields("IdMoneda")
            rsg.Fields("GlsObsAnula") = RsC.Fields("GlsObsAnula")
            rsg.Fields("TotalPrecioVenta") = RsC.Fields("TotalPrecioVenta")
    ...
    Else
        ...
        rsg.Fields("RucCliente") = ""
        rsg.Fields("GlsCliente") = ""
        rsg.Fields("IdMoneda") = ""
        rsg.Fields("GlsObsAnula") = ""
        rsg.Fields("TotalPrecioVenta") = 0
            
    End If
    RsC.Close: Set RsC = Nothing
    
    mostrarDatosGridSQL2 G, rsg, "Item", StrMsgError
    If StrMsgError <> "" Then GoTo Err
