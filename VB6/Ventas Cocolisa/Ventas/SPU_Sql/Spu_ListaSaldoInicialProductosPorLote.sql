...
GO
CREATE OR ALTER PROCEDURE dbo.Spu_ListaSaldoInicialProductosPorLote
(
    @VarIdEmpresa               CHAR(2),
    @VarIdAlmacen               CHAR(8),
...
BEGIN
    ...
    (
		...
		IdLote				VARCHAR(10) Not Null Default '',
		GlsLote				VARCHAR(200) Not Null Default '',
    ...
    (
        ...
        IdProvCliente, Ruc, GlsPersona, GlsDocReferencia, AbreUM, Almacen, GlsConcepto, Cantidad, VVUnit, SaldoInicial, Stock, CodigoRapido, IdLote, GlsLote
    )
    SELECT 
        ...
        Pr.CodigoRapido,
		iif(len(trim(isnull(vd.idlote,'')))  = 0 and vd.idSucursal = '08090001','10120001',  iif(vd.idSucursal = '08090002','10120002',vd.idlote)) IdLote,
		L.GlsLote
	...
	Inner Join Lotes L
	  On vd.IdEmpresa = L.IdEmpresa And vd.IdSucursal = L.IdSucursal
	  And iif(len(trim(isnull(vd.idlote,'')))  = 0 and vd.idSucursal = '08090001','10120001',  iif(vd.idSucursal = '08090002','10120002',vd.idlote)) = L.IdLote
    WHERE 
        ...
        AND Pr.estProducto = 'A' 
		--AND (N.Tipo = @VarTipoNivel OR @VarTipoNivel = '')
  --      AND (Pr.CodigoRapido = @VarCodigoRapido OR @VarCodigoRapido = '')
    GROUP BY VC.IdEmpresa,VC.IdSucursal,VC.IdAlmacen,A.GlsAlmacen, Pr.Codigorapido,Pr.IdProducto , Pr.GlsProducto, um.AbreUM,VD.IdProducto, Pr.GlsProducto,
		iif(len(trim(isnull(vd.idlote,'')))  = 0 and vd.idSucursal = '08090001','10120001',  iif(vd.idSucursal = '08090002','10120002',vd.idlote)),
		L.GlsLote
	...
	Select IdEmpresa,IdSucursal,IdValesCab,IdConcepto,TipoVale,FechaEmision,XIdProducto,IdProducto,GlsProducto,IdAlmacen,IdProvCliente,Ruc,GlsPersona,
	GlsDocReferencia,AbreUM,Almacen,GlsConcepto,Cantidad,VVUnit,SaldoInicial,Stock,IdLote,GlsLote
	From #TempSaldoIniPro
