...
public sealed class CierreProcesoRepository(IDbConnectionFactory connectionFactory) : ICierreProcesoRepository
{
    public async Task<CierreProcesoDto?> ObtenerAsync(int idEmpresa, short anio, CancellationToken cancellationToken = default)
    {
        await using var connection = connectionFactory.CreateConnection();
        await using var command = new SqlCommand("dbo.usp_CON_ObtenerCierreProceso", connection)
        {
    ...
    public async Task<CierreProcesoDto> GenerarAsync(GenerarCierreProcesoRequest request, CancellationToken cancellationToken = default)
    {
        await using var connection = connectionFactory.CreateConnection();
        await using var command = new SqlCommand("dbo.usp_CON_GenerarCierreProceso", connection)
        {
            CommandType = CommandType.StoredProcedure
        };
        ...
        return proceso ?? throw new InvalidOperationException("No se pudo obtener el proceso de cierre generado.");
    }

    public async Task EliminarAsync(int idEmpresa, short anio, string? usuarioRegistro, CancellationToken cancellationToken = default)
    {
        await using var connection = connectionFactory.CreateConnection();
        await using var command = new SqlCommand("dbo.usp_CON_EliminarCierreProceso", connection)
        {
            CommandType = CommandType.StoredProcedure
        };
    ...
    private static async Task<CierreProcesoDto?> LeerProcesoAsync(SqlCommand command, CancellationToken cancellationToken)
    {
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);

        CierreProcesoDto? proceso = null;
        if (await reader.ReadAsync(cancellationToken))
        {
            proceso = new CierreProcesoDto
            {
        ...
        if (await reader.NextResultAsync(cancellationToken))
        {
            ...
            {
                proceso.Detalles.Add(new CierreProcesoDetalleDto
                {
