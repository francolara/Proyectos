...
- `19/09/2026`: se incorpora trazabilidad de edición para Plan de cuentas, Centros de costo, Cuentas corrientes, Personas, Tipo de cambio, Orígenes, Cuentas destino, Asientos, Compras, Ventas y Caja y Bancos. El script incremental `20260919_AgregarTrazabilidadRegistro.sql` crea únicamente los campos faltantes de registro y actualización, sin completar la información histórica; cada SP de guardado registra explícitamente las altas, conserva su autor y fecha originales y asigna el usuario y la fecha de la última actualización.
...
- `07/09/2026`: las tablas temporales de `usp_CON_ReporteBalanceComprobacion`, `usp_CON_ReporteLibroMayor` y `usp_CON_CargarPlanCuentaDefaultEmpresa` declaran sus columnas textuales con `COLLATE DATABASE_DEFAULT`, evitando conflictos cuando `tempdb` y `Dbsisadm` usan intercalaciones diferentes. La aplicacion fija la cultura `es-PE` para mostrar en espanol los meses generados por el servidor y el Balance registra internamente los errores SQL sin exponer su detalle tecnico al usuario.
- `07/09/2026`: los contenedores compartidos de listas, detalles y reportes preservan su ancho dentro del espacio principal y habilitan desplazamiento horizontal tactil en pantallas pequenas. Esto evita que las tablas extensas queden recortadas y replica de forma transversal el comportamiento responsive que ya tenia el detalle de asientos. Ademas, las tarjetas de estado de las cabeceras (`Edicion`, `Nuevo`, periodos y equivalentes) se alinean al extremo derecho cuando la cabecera se apila en modo movil. La navegacion movil conserva el foco en el area de trabajo al abrir opciones del menu, detalles, formularios y retornos posteriores al registro o edicion, evitando que la nueva vista reaparezca al inicio del menu lateral.
