...
## Otros recordatorios
- Las tablas residen en la carpeta Basededatos/Dbsisadm/Tablas , (cada vez que se crea una tabla debe ser un archivo independiente). 
- Si se modifica o crea un procedimiento Basededatos/Dbsisadm/StoreProcedure ,  (cada vez que se crea una procedimiento se crea un archivo independiente, si se modifica se reemplaza en que ya existe).
- Si se Altera una estructura de una tabla o se agrega un insert o update para datos de una tabla  Basededatos/Dbsisadm/Script,(se pueden consolidar en un solo script , alter ,insert, update).
