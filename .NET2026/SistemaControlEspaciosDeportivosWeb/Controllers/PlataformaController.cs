    ...
    IHomeReferencialesExternosSyncService referencialesExternosSyncService) : Controller
{
    ...
    private async Task<PlataformaPortalConfigViewModel> CargarPortalConfigAsync()
    {
        ...
        {
            BeneficiosTitulo = (await Get("HOME_PORTAL_BENEF_TITULO")) ?? "Gestiona todo tu complejo deportivo desde un solo lugar",
            BeneficiosSubtitulo = (await Get("HOME_PORTAL_BENEF_SUBTITULO")) ?? "Reservas, tarifas, clientes, cobros y reportes para operar con control total.",
            Beneficio1Titulo = (await Get("HOME_PORTAL_BENEF_1_TITULO")) ?? "Reservas sin cruces",
            Beneficio1Detalle = (await Get("HOME_PORTAL_BENEF_1_DETALLE")) ?? "Evita horarios duplicados y bloquea automaticamente los espacios compartidos.",
            Beneficio2Titulo = (await Get("HOME_PORTAL_BENEF_2_TITULO")) ?? "Tarifas inteligentes",
            Beneficio2Detalle = (await Get("HOME_PORTAL_BENEF_2_DETALLE")) ?? "Define precios por espacio y turno, con tarifas especiales para domingos y feriados.",
            Beneficio3Titulo = (await Get("HOME_PORTAL_BENEF_3_TITULO")) ?? "Cobros y comprobantes",
            Beneficio3Detalle = (await Get("HOME_PORTAL_BENEF_3_DETALLE")) ?? "Registra adelantos y saldos, y emite comprobantes electronicos por cada reserva.",
            Beneficio4Titulo = (await Get("HOME_PORTAL_BENEF_4_TITULO")) ?? "Promociones y cupones",
            Beneficio4Detalle = (await Get("HOME_PORTAL_BENEF_4_DETALLE")) ?? "Crea descuentos y cupones para impulsar tus horarios de menor demanda.",
            Beneficio5Titulo = (await Get("HOME_PORTAL_BENEF_5_TITULO")) ?? "Clientes y reportes",
            Beneficio5Detalle = (await Get("HOME_PORTAL_BENEF_5_DETALLE")) ?? "Revisa ingresos diarios, cancelaciones y el consumo de cada cliente.",
            Beneficio6Titulo = (await Get("HOME_PORTAL_BENEF_6_TITULO")) ?? "Multiples deportes y espacios",
            Beneficio6Detalle = (await Get("HOME_PORTAL_BENEF_6_DETALLE")) ?? "Administra sedes, canchas y disciplinas deportivas desde un solo panel.",
