...
{
    public async Task<List<UsuarioNegocioItemViewModel>> UsuariosNegocioListarAsync(int negocioId)
    {
    ...
    public async Task<bool> UsuariosNegocioAsignarPorCorreoAsync(int negocioId, string correo, int rolNegocio, string usuario)
    {
        await using var cn = CreateConnection();
        await cn.OpenAsync();
